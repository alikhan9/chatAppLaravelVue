...
<template>
    <div class="px-5 py-5 flex justify-between items-center bg-white border-b-2 max-h-[9vh]">
        <Link :href="route('home')" class="font-semibold text-2xl text-blue-600 hover:text-blue-500">ChatApp
        </Link>

        <div class="w-1/2">
            <input name="search" type="text" v-model="filter" placeholder="search ..."
                class="rounded-2xl bg-gray-100 py-3 px-5 w-full" />
        </div>
        <div class="flex gap-4 items-center">
            <Dropdown :width="friend_requests.length + notifications.length > 0 ? 500 : 48
                ">
                <template #trigger>
                    <span class="inline-flex rounded-md mt-2">
                        <button @click="sendMarkNotif" type="button" class="relative">
                            <unicon width="40" height="40" name="users-alt" fill="gray" />
                        </button>
                    </span>
                </template>
                <template #content>
                    <div class="flex flex-col">
                        <div class="block w-full px-4 py-2 hover:cursor-pointer text-left text-sm leading-5 text-gray-700 hover:bg-gray-200 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
            ...
            <Dropdown :width="friend_requests.length + notifications.length > 0 ? 500 : 48
                ...
                <template #trigger>
                    <span class="inline-flex rounded-md mt-2">
                        <button @click="sendMarkNotif" type="button" class="relative">
                            <unicon width="40" height="40" name="bell" fill="royalblue" />
                            <p class="absolute -top-1 text-md right-[1px] text-white px-1 bg-blue-500 rounded-full">
                                {{
                ...
                <template #content>
                    <div class="max-h-[250px] overflow-auto">
                        <div v-for="(friend, index) in friend_requests" v-if="friend_requests.length > 0" :key="index"
                            :class="{
                        ...
                        <div v-for="(notification, index) in $page.props
                            .notifications" :key="index" v-if="notifications.length > 0" :class="{
        ...
        'border-b-2 border-dashed':
            notifications.length !== index + 1,
                        ...
                        <div v-if="notifications.length > 0" class="flex justify-center">
                            <button class="bg-red-500 px-4 py-2 min-w-full text-white" @click="sendDeleteNotifs">
                                Clear all
                            </button>
                        </div>
                        <div v-else class="p-2 text-center" v-if="friend_requests.length == 0">
                            No notification...
                        </div>
                    </div>
                </template>
